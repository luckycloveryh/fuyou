var RENDERER = {
    ...
    getFishColor: function () {
        var isDark = document.documentElement.getAttribute('data-scheme') === 'dark';
        return isDark ? 'rgba(255, 145, 190, 0.72)' : 'rgba(255, 125, 180, 0.62)';
    },
    getWaterColor: function () {
        var isDark = document.documentElement.getAttribute('data-scheme') === 'dark';
        return isDark ? 'rgba(255, 105, 170, 0.28)' : 'rgba(255, 180, 210, 0.36)';
    },
    render: function () {
        requestAnimationFrame(this.render);
        this.controlStatus();
        this.context.clearRect(0, 0, this.width, this.height);
        this.context.fillStyle = this.getFishColor();
        ...
        this.context.fillStyle = this.getWaterColor();
...
SURFACE_POINT.prototype = {
    ...
    SPRING_FRICTION: 0.94,
    WAVE_SPREAD: 0.16,
    ACCELARATION_RATE: 0.004,
...
FISH.prototype = {
    ...
    init: function () {
        ...
        this.vx = this.getRandomValue(1.2, 3.2) * (this.direction ? -1 : 1);

        if (this.renderer.reverse) {
            this.y = this.getRandomValue(this.renderer.height * 1 / 10, this.renderer.height * 4 / 10);
            this.vy = this.getRandomValue(0.7, 1.7);
            this.ay = this.getRandomValue(0.018, 0.07);
        } else {
            this.y = this.getRandomValue(this.renderer.height * 6 / 10, this.renderer.height * 9 / 10);
            this.vy = this.getRandomValue(-1.7, -0.7);
            this.ay = this.getRandomValue(-0.07, -0.018);
        }
        this.isOut = false;
        this.theta = 0;
        this.phi = 0;
